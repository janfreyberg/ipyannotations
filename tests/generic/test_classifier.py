...
from ipyannotations.generic import classifier


def test_submit_with_button(mocker):
    widget = classifier.ClassificationWidget(options=["a", "b"])
...
def test_number_keystrokes_trigger_submit(mocker):
    widget = classifier.ClassificationWidget()
    widget.options = ["a", "b"]
...
def test_max_buttons_switches_to_dropdown():
    widget = classifier.ClassificationWidget(max_buttons=6)
